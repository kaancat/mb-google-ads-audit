...
For example, you cannot write performance_analysis.json (Phase 3) if
tracking_audit.md (Phase 1) doesn't exist.
...
# Phase artifact files in order (8 phases: 0-7)
PHASE_ARTIFACTS = {
    0: ["discovery_brief.md"],
    1: ["tracking_audit.md"],
    2: ["structure_analysis.md"],
    3: ["performance_analysis.json"],
    4: ["keyword_audit.json"],
    5: ["ad_copy_audit.json"],
    6: ["recommendations.json"],
    7: ["audit_presentation.html"],
}
...
def find_client_dir_from_path(file_path: str) -> Path | None:
    ...
    # Check if path contains 'audits' directory
    if "audits" in parts:
        audits_idx = parts.index("audits")
        if audits_idx + 1 < len(parts):
            return Path(*parts[: audits_idx + 2])
...
def check_earlier_phases_complete(
    ...
    for phase in range(0, target_phase):
